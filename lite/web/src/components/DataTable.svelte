...
<div class="table-wrap"><table class="table"><thead><tr>{#each columns as column}<th style={`text-align:${column.align ?? "left"}`}>{column.label}</th>{/each}</tr></thead><tbody>{#each rows as row}<tr class:clickable={onRowClick} role={onRowClick ? "button" : undefined} tabindex={onRowClick ? 0 : undefined} on:click={() => onRowClick?.(row)} on:keydown={(event) => activate(row, event)}>{#each columns as column}{@const value = row[column.key]}<td style={`text-align:${column.align ?? "left"}`}>{#if isCell(value)}<span class={value.tone ?? ""}>{value.text}</span>{:else}{value ?? "—"}{/if}</td>{/each}</tr>{/each}</tbody>{#if totals}<tfoot><tr>{#each columns as column}{@const value = totals[column.key]}<td style={`text-align:${column.align ?? "left"}`}>{#if isCell(value)}<span class={value.tone ?? ""}>{value.text}</span>{:else}{value ?? ""}{/if}</td>{/each}</tr></tfoot>{/if}</table></div>
<style>
