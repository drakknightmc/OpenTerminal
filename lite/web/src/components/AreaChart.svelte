<script lang="ts">
  ...
  $: min = Math.min(...points, 0); $: max = Math.max(...points, 1); $: range = max - min || 1; $: line = points.length > 1 ? points.map((point, i) => `${i ? "L" : "M"}${(i / (points.length - 1)) * width},${height - ((point - min) / range) * height}`).join(" ") : ""; $: area = line ? `${line} L ${width},${height} L 0,${height} Z` : "";
