...
export async function handleCrypto(req: Request, url: URL): Promise<Response | null> {
  ...
  try {
    ...
    if (path === "/api/crypto") return json(await cached("crypto:markets", TTL.medium, () => crypto.getTopAssets(50)));
    ...
    if (path === "/api/crypto/ohlcv") {
      ...
      return json(await cached(`crypto:ohlcv:${symbol}:${timeframe}`, TTL.short, () => crypto.getOHLCV(symbol, timeframe)));
    }
